...
from segmentation_model import SegmentationModel  # New import
...
def main():
    ...
    sam_model_name = "sam2.1_b.pt"  # SAM 2.1 model name: "sam2.1_b.pt", "sam2.1_l.pt", "sam2.1_h.pt"
    
    # Device settings
    device = 'cuda'  # Device for running models
    ...
    if enable_segmentation:
        try:
            ...
            print(f"Initialized SAM 2.1 segmentation model ({sam_model_name})")
        except Exception as e:
            print(f"Error initializing segmentation model: {e}")
            print("Disabling segmentation due to initialization error")
    ...
    while True:
        ...
        try:
            ...
            segmentation_frame = frame.copy() if enable_segmentation else None
            ...
            if enable_segmentation and detections:
                try:
            ...
            if enable_segmentation and segmentation_frame is not None:
            ...
            # Add segment info if enabled
            if enable_segmentation:
                model_name = sam_model_name.split('.')[0]  # Extract sam2.1_b from sam2.1_b.pt
            ...
            # Show additional windows if segmentation is enabled
            if enable_segmentation and segmentation_frame is not None:
